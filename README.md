...
- **Live:** the table auto-refreshes every 15 s while the tab is open (and instantly on tab focus), so new sign-ups, scores and resumes appear without a manual reload. Set `NEXT_PUBLIC_ADMIN_REFRESH_MS` (e.g. `5000` for 5 s) to change the interval. A `Live` badge shows the last sync time, and a banner tells you whether you are viewing Supabase data or local demo data.
- Data comes from the same backend as the app: Supabase (`student_profiles_full` view) when configured, otherwise the local JSON store. The view is queried first; if it has not been created yet the dashboard falls back to joining the `profiles` / `resume_analyses` / `assessment_results` tables directly (and shows an explanatory banner). To read **every** student the admin should use a `SUPABASE_SERVICE_ROLE_KEY` — without it the anon key is used and Row Level Security can hide rows from the admin.
