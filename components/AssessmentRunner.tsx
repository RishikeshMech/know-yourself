...
function getSubProgress(
  ...
  if (stageId === 'english') {
    ...
    if (subIdx === 1) {
      const tasks: any[] = bank.english.speaking.tasks
      const answered = tasks.filter(t => answers[t.id + '_audio']).length
...
function prevLocation(STAGES: StageDef[], stage: number, sub: number): { stage: number; sub: number } | null {
...
function locationLabel(STAGES: StageDef[], stage: number, sub: number): string {
...
export function AssessmentRunner({ config }: { config: AssessmentConfig }) {
  ...
  // store slice, which the rest of the app still reads.
  ...
  const gridCfg = bank.cognitive?.grid || { rounds: 0, patternSize: 0, gridCells: 0, showMs: 0, note: '' }
  ...
  useEffect(() => {
    ...
    const tryLoad = () => {
      ...
      attempts += 1
      if (attempts >= 8) { router.replace(config.fallbackRoute()); return }
  ...
  useEffect(() => {
    if (sid) {
      localStorage.setItem(KEYS.answers(sid), JSON.stringify(answers))
  ...
  useEffect(() => { if (sid) localStorage.setItem(KEYS.ai(sid), JSON.stringify(aiResults)) }, [aiResults, sid])
  ...
  useEffect(() => {
    if (STAGES[stage]?.id !== 'cognitive' || sub !== 0 || !gridCfg.gridCells) return
  ...
  const goNext = () => {
    const n = nextLocation(STAGES, stage, sub)
    if (n) navigateTo(n.stage, n.sub, 'next')
  }
  const goPrev = () => {
    const p = prevLocation(STAGES, stage, sub)
  ...
  useEffect(() => {
    ...
    const next = nextLocation(STAGES, stage, sub)
    if (!next) {
      showToast('Section complete ✓ — review your answers and submit when ready.')
      return
    }
    const fromLabel = locationLabel(STAGES, stage, sub)
    const toLabel = locationLabel(STAGES, next.stage, next.sub)
  ...
  const review = useMemo(() => config.buildReview(bank, answers), [config, bank, answers])
        ...
        if (sub === 1) {
          return (
            <div className="space-y-5">
              ...
              {speakingCount > 0 && speakingCount < bank.english.speaking.tasks.length && (
                <div className="mt-1 text-xs font-semibold text-slate-500">{speakingCount} of {bank.english.speaking.tasks.length} speaking tasks recorded.</div>
              )}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs font-black tracking-wide flex items-center gap-2">
                    <span>{isA2 ? 'Debugging Lab — In-built Compiler' : 'AI-Assisted Debugging Stage'}</span>
                  ...
                  <div className="text-[11px] text-indigo-200 mt-0.5">
                    Proctored assessment — do not switch browser tabs. Write your corrected code, run the hidden tests in the built-in compiler, and use the AI Assistant below for explanations or a code review. <b className="text-white">You have 5 assistant prompts for this task — use them wisely.</b>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between flex-wrap gap-2 border-b border-slate-200/70 pb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[11px] font-mono px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
                    {d.lang || (d.id.startsWith('AD2') ? 'JavaScript' : 'Python')}
                  </span>
                </div>
              </div>
  ...
  const subs = STAGES[stage].sub
  const nextLoc = nextLocation(STAGES, stage, sub)
  const prevLoc = prevLocation(STAGES, stage, sub)
  const nextLabel = nextLoc ? locationLabel(STAGES, nextLoc.stage, nextLoc.sub) : ''
  const prevLabel = prevLoc ? locationLabel(STAGES, prevLoc.stage, prevLoc.sub) : ''
    ...
    <div className="min-h-screen text-slate-800">
      ...
      <div className="sticky top-0 z-30 border-b border-white/60 bg-white/70 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <Logo height={32} />
            <span className="hidden sm:inline font-extrabold text-slate-900 text-sm">{config.title}</span>
            {isA2 && (
