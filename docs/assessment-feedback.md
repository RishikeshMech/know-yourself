...
The candidate must select 1–5 stars and review a 10–1,000-character message. Every rating supplies editable wording. Only a successful `/api/feedback` response clears the pending step. The API stores the complete submission in Supabase's `feedback_submissions` table (and keeps the local demo-store fallback). Errors retain the form for retry.
...
AI is opt-in; only the rating and comments go to the provider. Assessment feedback is sent only to `/api/feedback` and stored in Supabase; it is no longer sent to Formspree.
...
- Mock `/api/feedback` failure: form stays populated and continuation stays blocked.
- Mock `/api/feedback` success: thank-you state appears; dashboard is available without repeating feedback.
