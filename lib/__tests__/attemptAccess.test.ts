...
test('an existing local score sends the visitor to their profile', () => {
  assert.equal(resolveInstructionsRedirect(store({ [SCORES_KEY]: '{"total":812}' })), '/profile')
})

test('a submitted or expired session sends the visitor to their profile', () => {
  assert.equal(resolveInstructionsRedirect(store({ [SESSION_KEY]: session('submitted') })), '/profile')
  assert.equal(resolveInstructionsRedirect(store({ [SESSION_KEY]: session('expired') })), '/profile')
})
